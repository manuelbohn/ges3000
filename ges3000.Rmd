...
#### number of differetn types of gestures used?
```
...
gram_plot_12 <- gram_data%>%
  ...
  mutate(typediff_1 = gsub("hol","enact",typediff_1),
         ...
         typediff_1 = gsub("partseq","atom_part",typediff_1),
         typediff_1 = plyr::mapvalues(typediff_1,c("numges","sizeges","moveges","seq"),rep("atom",4)))%>%
...
gram_plot_13 <- gram_data %>%
   filter(compr == 1,
          diff == "yes",
        predicate != "small",
         predicate != "static",
         predicate != "one")%>%
  select(condition,mod_or_add,sim_or_seq, hol_or_atom)%>%
  gather(feature,presence, -condition)%>%
  group_by(condition, feature,presence)%>%
  summarise(count = n())%>% 
  mutate(perc=count/sum(count))
...
ggplot() +
  geom_bar(data = gram_plot_13, stat = "identity" ,aes(x = feature, y = perc, fill = factor(presence)))+
  facet_grid(condition~.)+
  scale_y_continuous(name = "Proportion feature present")+
  xlab("Feature")+
  ggtitle("Gesture features")+
  scale_fill_pander(name = "Feature present", breaks=c("0","1"), labels = c("no","yes"))+
  theme_few()+
  coord_flip()
