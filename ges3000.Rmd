...
# model 
m8 <- glmer(corr ~ age+phase + (trial|id) + (1|stimulus), family = binomial, data = d8.m, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
# summary
summary(m8)
# p values
drop1(m8, test = "Chi")
...
ggplot() +
  ...
  geom_pointrange(data = d6.p3,aes(x = stimulus, y = mean, col = Age,ymin = ci_lower, ymax = ci_upper),position = position_dodge(width = .5), size = 1) + 
  geom_hline(yintercept = 0.5, lty=2)+
  #facet_grid(~block)+
  guides(alpha = F)+
...
ggplot() +
  ...
  geom_pointrange(data = d7.p3,aes(x = stimulus, y = mean, col = Age,ymin = ci_lower, ymax = ci_upper),position = position_dodge(width = .5), size = 1) + 
  geom_hline(yintercept = 0.5, lty=2)+
  facet_grid(~day,scales = "free_x", space = "free_x")+
  guides(alpha = F)+
  scale_y_continuous(name = "Proportion Later Chosen", limits = c(-0.05,1.05))+
