...
d1.m <- d1 %>%
 ...
 m1 <- glmer(prompts ~ blocktrial*age + (blocktrial|dyad_id), family = binomial, data = d1.m,
...
m2 <- glmer(prompts ~ block*age*blocktrial + (blocktrial + block||id), family = binomial, data = d2.m,
        control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
ggplot(d5.p, 
       aes(x = block, y = prompts, col = Age)) +
  geom_point(aes(group= Age),position = position_dodge(width = .5))+
  #facet_grid(~block)+
  scale_y_continuous(name = "Dyads with Prompts", limits = c(0,12), breaks = c(0,2,4,6,8,10,12))+
  theme_few() + 
  guides(size=FALSE)+
...
ggplot() +
  geom_jitter(data = d6.p, aes(x = Age, y = corr, col = Age, alpha = .2), width = .1,height = .05)+
  geom_pointrange(data = d6.p1,aes(x = Age, y = mean, col = Age,ymin = ci_lower, ymax = ci_upper),position = position_dodge(width = .5)) + 
...
ggplot() +
  ...
  geom_pointrange(data = d6.p3,aes(x = stimulus, y = mean, col = Age,ymin = ci_lower, ymax = ci_upper),position = position_dodge(width = .5)) + 
  geom_hline(yintercept = 0.5, lty=2)+
  #facet_grid(~block)+
  guides(alpha = F)+
...
ggplot() +
  ...
  geom_pointrange(data = d7.p1,aes(x = day, y = mean, col = Age,ymin = ci_lower, ymax = ci_upper),position = position_dodge(width = .5)) + 
  geom_hline(yintercept = 0.5, lty=2)+
  #facet_grid(~day,scales = "free_x", space = "free_x")+
  guides(alpha = F)+
  scale_y_continuous(name = "Proportion Later Chosen", limits = c(0,1))+
  theme_few() + 
  scale_colour_solarized()
...
ggplot() +
  ...
  geom_pointrange(data = d7.p3,aes(x = stimulus, y = mean, col = Age,ymin = ci_lower, ymax = ci_upper),position = position_dodge(width = .5)) + 
  geom_hline(yintercept = 0.5, lty=2)+
  facet_grid(~day,scales = "free_x", space = "free_x")+
  guides(alpha = F)+
...
# are kids better in test compared to pretest when seeing abstract gestures?
d8.m <- d8  %>%
  filter(condition == "Abstract") %>%
  mutate(trial = scale(as.numeric(trial), center = TRUE, scale=TRUE))

# model 
m4 <- glmer(corr ~ age+phase + (trial|id) + (1|stimulus), family = binomial, data = d8.m, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
ggplot() +
  ...
  geom_pointrange(data = d8.p1,aes(x = condition, y = mean, col = Age,ymin = ci_lower, ymax = ci_upper),position = position_dodge(width = .5)) + 
