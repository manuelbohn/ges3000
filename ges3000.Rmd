...
Data and analysis scripts can be found online at ``htps://github.com/manuelbohn/ges3000``. To improve readability, we only report a small set statistics in the text itself (estimates and p-values for model parameters and p-values for non-parametric tests). For additional information as well as further analysis, we ask the reader to directly consult the analysis scripts online. All tests were run in R [@R-base] and for models we used the function glmer of the lme4 package [@R-lme4]. Random effect structures were maximal whenever possible. Model p-values were based on likelihood ratio tests [@dobson2008introduction] and were computed via the function ``drop1``. We removed non-significant interaction terms from models unless they were the target of the inference.
