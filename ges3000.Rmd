...
#model does not converge with random intercept for block
...
```
Figure \@ref(fig:plot6) summarizes the results of this experiment. Children of both age groups successfully differentiated between different instantiations of the same referent in the majority of trials. We coded three different gesture differentiation types (see also Figure \@ref(fig:difftypes). The code "enact" reflected trials in which children enacted the way the referent moved at the same time as they were depicting the referent. For example, to depict a turning monkey, the child scratched their head (for "monkey") while turning (for "turn"). One child used the number of repetitions of the gesture to indicate movement ("running"). For example, they would make one somersault for a static ball and multiple somersaults for the "running" ball. We used the code "repeat" to capture this. The code "combinatorial" denotes combinatorial gesture sequences. For example, children would first scratch their head (for "monkey") and then, in a seperate gesture, move their index finger in a circular motion in front of their body. Enact gestures were the most frequent type. Children were fairly consistent in their use of gesture types and the majority of children used only a single type (see Figure \@ref(fig:varwithin). The right graph in Figure \@ref(fig:plot6) (C) shows the proportion of combinatorial gesture sequences. We found that differentiation greatly enhanced gesture comprehension ($\beta$ = `r round(summary(mmove.1)$coefficients[3,1], 2)`, se = `r round(summary(mmove.1)$coefficients[3,2], 2)` *p* = `r sub('^(-)?0[.]', '\\1.', round(pmmove.1[3,4], 3))`, see also middle plot in Figure \@ref(fig:plot5). Eight-year-olds' gestures were comprehended slightly better($\beta$ = `r round(summary(mmove.1)$coefficients[2,1], 2)`, se = `r round(summary(mmove.1)$coefficients[2,2], 2)` *p* = `r sub('^(-)?0[.]', '\\1.', round(pmmove.1[2,4], 3))`). There was no effect of trial ($\beta$ = `r round(summary(mmove.1)$coefficients[4,1], 2)`, se = `r round(summary(mmove.1)$coefficients[4,2], 2)` *p* = `r sub('^(-)?0[.]', '\\1.', round(pmmove.1[4,4], 3))`) or block ($\beta$ = `r round(summary(mmove.1)$coefficients[5,1], 2)`, se = `r round(summary(mmove.1)$coefficients[5,2], 2)` *p* = `r sub('^(-)?0[.]', '\\1.', round(pmmove.1[5,4], 3))`).
...
The rate of differentation and use of combinatorial gesture sequences were highly autororrelated within children. Most individuals who differentiated in one trial, differentiated in all trials and most children who used one combinatorial gesture sequence, used them in all trials. From a modeling perspective, this lead to extreme estiamtes for child specific random intercepts (`(1 | id)`) and with that, to a very large variance on the intercept term, which overshadowed any other structure in the data. To avoid this variance inflation, we classified children based on their mean proportion in the response. That is, children who differentiated in more than 50% of trials were coded as 1 and those who differentiated in less than 50% as 0. We then used GLMs to model the data. 
...
mdiff_8 <- glm(diff ~ condition , family = binomial, data = mdiff_8_data)
...
This analysis include all data for 8yo. We found no effect of condition ($\beta$ = `r round(summary(mdiff_8)$coefficients[2,1], 2)`, se = `r round(summary(mdiff_8)$coefficients[2,2], 2)` *p* = `r sub('^(-)?0[.]', '\\1.', round(pdiff_8[2,5], 3))`).
...
Here we asked what influenced children's use of combinatorial gesture sequences as opposed to holisitc gestures. Given that this was a question about what type of solution children used, we only selected trials in which children differentiated. As noted above, for movement, number and size, the differentiation type was a realtive code and we only coded it for the newly introduced pictures (i.e. big, moving or multiple referents). Therefore, we only select trials for which we coded a differentiation type. All gesture types that were not coded as "combinatorial" or "combinatorial_part" were classified as "holisitc gestures". Due to the autocorrelation in children's responses (see [above](#part-2-overall-analysis)), the dependent variable in the follwoing models was whether the child used combinatorial gesture sequences in the majority of trials.

We did not expect a difference between the conditions movement, number and size in the analysis for both age groups. However, we did predict a condition effect when including  transitive actions for 8yo. Depicting a single property of a referent (its movement, number or size) is possible by modifying an already existing gesture for the referent. On the other hand, expressing a transitive realtion between two agents requires specifying who the agent and who the patient is. Though possible, enacting such a relation in a single holistic gesture is difficult. Instead, we expected children to convert the transitive relation into a temporal one and specify the interactional roles via the order in which they produce the gestures for agent and patient. Doing so results in a gesture sequence. We therefore expected children to use more combinatorial gestures in transitive 2 and 3 compared to all other conditions. 
...
mcomb_age_data <- comb_age_data%>%
  mutate(condition = relevel(condition, ref = "size"))%>%
  group_by(id, Age, condition)%>%
...
emmeans(mcomb_8yo, pairwise ~ condition, type = "response")$contrasts

xxxx <- emmeans(mcomb_8yo, spec ="condition")

drop1(mcomb_8yo,test = "Chisq")

```

exclude trans 3 because all 1 and therefore estimation impossible. 

GLMMs with id random effect would not converge. Again we aggregated data for each individual and ran a simple lm. Main effect of condition. Transitive 2 and size are not different from transitive 3 but movement and number are.
...
```{r comb, fig.cap="Proportion of trials in which children used a combinatorial gesture sequence by condition and age."}
...
Combinatorial gestures are more effortful in that children need to depict several elements after one another. Here we look at the influence of combinatorial gestures on comprehension. 
...
since analysed comprehension, we can use a glmer again, as there was more variation in this response. 
...
# random slope for age on stimulus leads to convergence issues in drop1 and is thereofre removed
...
Both types are understood very well but combinatorial gestures are understood better. This is significant but the change in proportion is really small. To follow up on this, one should increase the complexity of the pictures. No effect of age on gesture comprehension of differentiated gestures. Effect of condition (see figure)
...
Here we ask whether the the internal structure children impose on their gesture sequences is influenced by their native language. It might be that children first translate the picture into a linguistic representation and then translate the elements of this representation into gestures. If this were the case, this would be evidence against the idea that children structure their gestures for communicative reasons as they would get the structure "for free" by relying on the structure of their native language. The corresponding prediction would be that the word order in children's combinatorial gestures should mostly, if not always, follow the order used in their native language (German). 

The strong version of this claim could already be refuted based on the results discussed so far. The majority of children preferred holistic gestures in number and movement. If gesture production was always mediated by a lingusitic representation, combinatorial gestures should have been the default across conditions. The weaker claim we investiagte here is that when children use a gesture sequence, they structure it according to their native language. 

Note that a strong bias in favor of a particular word order could have other origins [see e.g. @gibson2013noisy; @goldin2008natural; @ozccalicskan2016does; @langus2010cognitive] and is not direct evidence for the influence of the native language. However, finding that children do *not* have a particularly strong bias for the structure in their native language, is good evidence against such an influence. 

We excluded gestures coded as "combinatorial_part". While we do think that these gestures are combinatorial in that the meaning expressed by the sequence depends on its internal structure, the fact that the predicate was depicted together with the subject and/or the object makes an ordering of these components difficult. Consequently, we could only look at the order of S and O in these gestures. Here we would expect a strong bias in favor of an SO order independent of German word order [see e.g. @dryer2002case; @gildea2010grammars; @greenberg2010language].
...
The data remaining after subsetting included more 8yo and also more data from the transitive conditions. We therefore did not analyse the influence of age or condition on the choice of a particular word order. Similar to differentiation and the use of combinatorial gesture sequences, children's responses were highly autocorrelated. Like in previous analysis, we therefore classified each child as either using mostly German vs. using mostly non-German word order. To test for a bias for a particular word order, we tested whether the intercept in a GLM with German word order (1/0) as dependend variable was different from 0 (corresponding to 50%). We found a small, but non-significant bias for German word-order ($\beta$ = `r round(summary(m_worder)$coefficients[1,1], 2)`, se = `r round(summary(m_worder)$coefficients[1,2], 2)` *p* = `r sub('^(-)?0[.]', '\\1.', round(summary(m_worder)$coefficients[1,4], 3))`, see also Figure \@ref(fig:wordorder)). As mentioned above, the final sample included a larger proportionn of children from transitive 2 and 3. Recall that the stimuli in these conditions were semantically reversible actions between two agents. In previous work, these type of scenarios have prompted SPO word order in speakers of languages with different word orders. In addition to German word order, this might explain the small bias towards SPO we observed here [@gibson2013noisy].
...
### Exploratory: Efficiency of gesture use

purely exploratory, maybe interesting
...
In the case of transitive actions, all pictures have to identified individually. However, in the case of transitive 2, the patient might be omitted. Therefore, if children were efficient, they should use fewer elements in transitive 2 compared to transitive 3. 
...
m.eff_comb.1 <- glmer(use_typediff ~ 1 + (1 | id) + (age_c | stimulus), family = binomial, data = d.eff_comb, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

summary(m.eff_comb.1)

# test if efficiency is further influence by age and condition 
m.eff_comb.2 <- glmer(use_typediff ~ Age + condition + (1 | id) + (age_c | stimulus), family = binomial, data = d.eff_comb, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
This is not influenced by other variables
