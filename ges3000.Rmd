...
Two disperate literatures, one looking at how children without a language model create their own structured communication system and the tother looking at how communication changes and structure emerges over repeated interactions in adults. Here we show that children who have already mastered one communciation system, wuickly switch modalities to create a new one. Furthermore, we show that these newly created communication systems already possess some of the core properties of language.
...
See 1.1.1. We excluded trials with the empty picture. These trials will be analysed speparately in the next section. 
...
m7 <- glmer(corr ~ Age+day+(Age|rater)+(1|stimulus), family = binomial, data = d7, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
ggplot() +
  ...
  geom_pointrange(data = d6.p3,aes(x = stimulus, y = mean, col = Age,ymin = ci_lower, ymax = ci_upper),position = position_dodge(width = .5), size = .5, alpha = .5) + 
