...
m1 <- d1 %>%
  ...
  summarise(prompts = sum(prompts))%>%
  mutate(prompts = ifelse(prompts > 0,1,0)) %>%
  glmer(prompts ~ blocktrial*age + (blocktrial|dyad_id), family = binomial, data = .,
...
m2 <- d2 %>%
  ...
  glmer(prompts ~ block*age*blocktrial + (blocktrial + block||id), family = binomial, data = .,
        control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
ggplot() +
  geom_jitter(data = d4.p, aes(x = block, y = comp, col = Age, alpha = .2), width = .2)+
...
ggplot(d5.p, 
  ...
  geom_point(aes(group= Age, size = 3),position = position_dodge(width = .5))+
  #facet_grid(~block)+
  scale_y_continuous(name = "Dyads with Prompts", limits = c(0,12), breaks = c(0,2,4,6,8,10,12))+
  theme_few() + 
  guides(size=FALSE)+
...
ggplot() +
  geom_jitter(data = d6.p, aes(x = Age, y = corr, col = Age, alpha = .2), width = .1)+
...
ggplot() +
  geom_jitter(data = d6.p2, aes(x = stimulus, y = corr, col = Age, alpha = .2), width = .1)+
