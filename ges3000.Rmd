...
mmove.1 <- glmer(compr ~ Age + diff + trial + (trial | id) + (1 | agent), family = binomial, data = dmove.m1, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
mnum.1 <- glmer(compr ~ Age + diff  + trial + (trial | id) + (1 | stimulus), family = binomial, data = dnum.m1, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
msize.1 <- glmer(compr ~ Age + diff + trial + (trial | id) + (1 | stimulus), family = binomial, data = dsize.m1, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
In the following provide some additional analysis based on the data from part 2. 

### Age differences

For the age related analysis we only used data from 2.1, 2.2 and 2.3.

#### Differentiation by age

```{r diffage data, cache = T}
...
mdiff_age <- glmer(diff ~ Age + condition + block + trial +  (trial | id) + (1 | condition/stimulus), family = binomial, data = mdiff_age_data, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
Results: Children differentiate more when they are older and in later trials (but not )

#### Combinatorial solutions by age

```{r diffcomb data, cache = T}
...
mcomb_age <- glmer(hol_or_atom ~ Age + condition + (1|id) + (1 | condition/stimulus), family = binomial, data = mcomb_age_data, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
# fitting model with data aggregated for each individual
...
mcomb_age_a <- lmer(mean_atom ~ Age + condition + (1 | condition), data = mcomb_age_data_a)
...
dens(x, adj=0.5 , norm.comp=T)
```

Here we heavily subset the data, that's why trial and block as a fixed and random effects makes less sense. they are therefore removed

Model suggests not effect of age on combinatorial gesture use. However, the variance on the random effect for id is gigantic. The reason might be that almost all cases are extreme, in the sense that most individuals either always use combinatorial solutions or never. This results in extreme estimates for individual intercepts. This variation over shadows differences between individuals due to age. 

When excluding random effect for id we get a large effect of age

#### Gesture 

### Efficiency of gesture use

How efficiently do children use their gestures? Are they trying to reduce effort by only marking the elemnts that are necessary?

#### Marked gestures for combinatorial solutions
...
m.eff_comb.1 <- glmer(use_typediff ~ 1 + (1 | id) + (1 | condition/stimulus), family = binomial, data = d.eff_comb, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

summary(m.eff_comb.1)

# test if efficiency is further influence by age and condition 
m.eff_comb.2 <- glmer(use_typediff ~ Age + condition + (1 | id) + (1 | condition/stimulus), family = binomial, data = d.eff_comb, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
#### Number of elements in transitive 

Relatedly we can look at the number of elements used by 8yo in trans 2 vs trans 3. If they care about efficiency, they should use fewer elements in trans 2 than trans 3.
...
m.el_by_con <- glmer(nr_elements ~ condition + (trial | id) + (1 | condition/stimulus), family = poisson, data = elements_by_condition_md, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
ggplot() +
  geom_jitter(data = elements_by_condition, aes(x = condition, y = nr_elements), alpha = .5, width = .1,height = .05)+
  geom_pointrange(data = elements_by_condition_2,aes(x = condition, y = mean,ymin = ci_lower, ymax = ci_upper),position = position_dodge(width = .5), size = 0.7) +
  guides(alpha = F)+
...
m.hol_comb <- glmer(compr ~ typediff_1 + ( 1 | id) + (1 | condition/stimulus), family = binomial, data = d.hol_comb, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
Do not include age in this analysis to use the data from transitive 2

Both types are understood very well but combinatorial gestures are understood better.
...
When children use gesture sequences, the question is whether they use their native language to structure and later communicate about the content of the picture. If we would find that they would exclusively do so, the idea that children are newly creating grammatical constructions would be obsolete. They would merely translate the proporties. We therefore looked at the order of elemnts for all gesture squences. For movement, number and size, the corresponding Geerman word order would be P-S ("hüpfender Elefant", "viele Hämmer", "kleine Gabel"). In the case of transitive actions, it would be SPO ("Hase jagt Katze"). 


We exclude "combinatorial_part" for this analysis. While we think that these gestures are combinatorial in that the meaning expressed by the sequence depends on its strcuture, the fact that the predicate was depicted together with the subject and/or the object makes an ordering of these components difficult. For these gestures we could only look at the order of S and O. Here, we would expect a strong bias in favor of an SO order for reasons other than German word order (see e.g. XXX-Lakoff?).

```{r}
### compare proportion of use in in two conditions? 
...
yy <- glmer(lang_con_order ~ 1 + (1 | id) + (1 | condition/stimulus), family = binomial, data = xx, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
Not more likely to use a German than a non-German word order. Key characteristic is variability. There
...
```{r}
### fitting bayesian model with constrained priors
# library(brms)
# 
# mcomb_age_b <-
#   brm(hol_or_atom ~
#         Age + condition +
#         (1|id) + (1 | condition/stimulus),
#       family = bernoulli(),
...
# 
# 
# model_weights(mcomb_age_b, mcomb_age_b_null, weights = "waic")
# 
# bayes_factor(mcomb_age_b, mcomb_age_b_null)
# 
# WAIC(mcomb_age_b, mcomb_age_b_null)
# 
# summary(mcomb_age_b)
# 
# plot(mcomb_age_b)
...
#            set_prior("normal(0,1)", class = "sd", coef = "Intercept", group = "id"),
#            set_prior("normal(0,1)", class = "sd", group = "id"),
#             set_prior("normal(0,1)", class = "sd", coef = "Intercept", group = "condition"),
#            set_prior("normal(0,1)", class = "sd", group = "condition"))
# 
# 
# get_prior(hol_or_atom ~
#         Age + condition +
#         (1|id) + (1 | condition/stimulus),
#       family = bernoulli(),
#       data = mcomb_age_data)
