...
m2 <- glmer(prompts ~ block*age*blocktrial + (blocktrial|id) + (1|stimulus) , family = binomial, data = d2.m,
...
m4 <- glmer(comp ~ age+block+trial+ (trial|dyad_id) + (1|stimulus), family = binomial, data = d4.m,
        control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
For each dyad we selected the video of the last gesture for a picture from the PC during spontaneous and the first gesture for the same picture from the PC (former CC) during uptake. That is, we chose the two depictions of the same picture by the two children that were closest in time. We used E-Prime (version 2.0.10.356) to set-up the rating procedure. On each trial, the rater was shown a sample video on top of the screen and two comparison videos on the bottom, one on the left and one in the right. The sample showed a child during the spontaneous part. One of the comparison videos showed the child from the *same* dyad during uptake whereas the other video showed a child of the same age and sex from a *different* dyad, also during uptake, also producing a gesture for the same picture. On each trial, the comparison video was randomly selected from the pool of suitable videos. Furthermore, the position of the comparison videos on the screen (left or right) was randomly determined. Raters could watch and re-watch each video multiple times. Each video was numbered (sample video was always no. 1) and raters were asked: "Which video is more similar to 1". They made a choice by pressing the respective number on the keyboard. Raters went through the videos in blocks. Each block consisted of all videos from a given age and sex for a particular picture. The order of blocks was randomized across raters. We only used videos corresponding to trials in which the child produced a gesture without prompts. This resulted in a total number of 97 sample videos.
...
# model 
m6 <- glmer(corr ~ age + (1 | rater) + (1 | video), family = binomial, data = d6.m, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
# summary
...
# model 
m6.2 <- glmer(corr ~ age + (1| rater) + (1 | video), family = binomial, data = d6.m2, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
# summary
...
m7.1 <- glmer(abs ~ age + effort + (1 | rater) + (1 | video), family = binomial, data = d7.m1, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
m7.2 <- glmer(abs ~ age + effort + (1 | rater) + (1 | video), family = binomial, data = d7.m2, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
mmove.1 <- glmer(compr ~ Age + diff + trial + block + (trial | id) + (1 | agent), family = binomial, data = dmove.m1, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

pmmove.1 <- drop1(mmove.1, test = "Chisq")

```
Figure \@ref(fig:plot6) summarizes the results of this experiment. Children of both age groups successfully differentiated between different instantiations of the same referent in the majority of trials. We coded three different gesture differentiation types (see also Figure \@ref(fig:difftypes). The code "enact" reflected trials in which children enacted the way the referent moved at the same time as they were depicting the referent. For example, to depict a turning monkey, the child scratched their head (for "monkey") while turning (for "turn"). One child used the number of repetitions of the gesture to indicate movement ("running"). For example, they would make one somersault for a static ball and multiple somersaults for the "running" ball. We used the code "repeat" to capture this. The code "combinatorial" denotes combinatorial gesture sequences. For example, children would first scratch their head (for "monkey") and then, in a seperate gesture, move their index finger in a circular motion in front of their body. Enact gestures were the most frequent type. Children were fairtly consistent in their use of gesture types and the majority of children used only a single type (see Figure \@ref(fig:varwithin). The right graph in Figure \@ref(fig:plot6) (C) shows the proportion of combinatorial gesture sequences. We found that differentiation greatly enhanced gesture comprehension ($\beta$ = `r round(summary(mmove.1)$coefficients[3,1], 2)`, se = `r round(summary(mmove.1)$coefficients[3,2], 2)` *p* = `r sub('^(-)?0[.]', '\\1.', round(pmmove.1[3,4], 3))`, see also middle plot in Figure \@ref(fig:plot5).
...
```{r difftypes, echo = F, fig.cap="Frequency of gesture types in each condition. For size, number and movement, the proportion collapsesa cross ages. The gesture types *combinatorial* and *combinatorial_part* indicate combinatorial gesture sequences, all other types denote holistic gestures", fig.width = 10, fig.height = 5, fig.scale = 1.5}
...
```{r varwithin, echo = F, fig.cap="Number of different gesture types used by individual children in each condition. Error bars are 95% confidence intervals based on non-parametric bootstraps.", fig.width = 10, fig.height = 5, fig.scale = 1.5}
...
ggplot() +
  geom_jitter(data = var_with_plot, aes(x = condition, y = diff_var, col = condition,  alpha = .2), width = .2,height = .0)+
  geom_pointrange(data = var_with_plot2,aes(x = condition, y = mean,col = condition, ymin = ci_lower, ymax = ci_upper),position = position_dodge(width = .5), size = 0.7) +
  guides(alpha = F, col = F)+
...
mnum.1 <- glmer(compr ~ Age + diff  + trial + block + (trial | id) + (1 | stimulus), family = binomial, data = dnum.m1, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

pmnum.1 <- drop1(mnum.1, test = "Chisq")

```

Figure \@ref(fig:plot7) summarizes the results of this experiment. Children of both age groups successfully differentiated between different instantiations of the same referent in the majority of trials. We coded five different gesture differentiation types (see also Figure \@ref(fig:difftypes). Like for movement, the code "combinatorial" denotes combinatorial gesture sequences. For example, children would first hold up their hand with the index finger close to the thumb (for "small") and then, in a seperate gesture, open and close their hand in front of their mouth (for "duck"). The code "tempo" reflected trials in which children used the speed in which they depicted the referent to indicate its numerosity. For example, for one hammer, the child would slowly hammer three times while for many hammers, the child would quickly hammer three times. The code "repeat" captured trials in which children would use the number of repetitions to depict numerosity. For example, the child would hammer once for one hammer and multiple times for many hammers. The code "bimanual" was used when children used the number of arms involved in the gesture to indicate numerosity. For example, the child would hammer with one arm for one hammer and with both arms for many hammers. Finally, the code "position" was used when children repeated the same gesture in multiple positions to refer to multiple referents. For example, for one hammer, the child id a hammering gesture while staying in one location and for many hammers, they did the same gesture multiple times but moved to a different location each time. The most frequent gesture type was "bimanual". The variability with respect to gesture type between children was also evident within children. Most children used more than one different gesture type in order to differentiate (see Figure \@ref(fig:varwithin)). The right graph in Figure\@ref(fig:plot7) (C) shows the proportion of "combinatorial" gestures. Like for movement, we looked at the effect of differentiation and found that differentiation greatly enhanced gesture comprehension ($\beta$ = `r round(summary(mnum.1)$coefficients[3,1], 2)`, se = `r round(summary(mnum.1)$coefficients[3,2], 2)` *p* = `r sub('^(-)?0[.]', '\\1.', round(pmnum.1[3,4], 3))`, see also middle plot in Figure \@ref(fig:plot7)). The model also suggested higher rates of comprehension for gestures produced by 8-year-olds ($\beta$ = `r round(summary(mnum.1)$coefficients[2,1], 2)`, se = `r round(summary(mnum.1)$coefficients[2,2], 2)` *p* = `r sub('^(-)?0[.]', '\\1.', round(pmnum.1[2,4], 3))`) and better comprehension of gestures in later trials ($\beta$ = `r round(summary(mnum.1)$coefficients[4,1], 2)`, se = `r round(summary(mnum.1)$coefficients[4,2], 2)` *p* = `r sub('^(-)?0[.]', '\\1.', round(pmnum.1[4,4], 3))`). 
...
msize.1 <- glmer(compr ~ Age + diff + trial + block + (trial | id) + (1 | stimulus), family = binomial, data = dsize.m1, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

pmsize.1 <- drop1(msize.1, test = "Chisq")

```
Figure \@ref(fig:plot8) summarizes the results. Again, children of both age groups successfully differentiated between different instantiations of the same referent in the majority of trials. We coded three different gesture differentiation types (see also Figure \@ref(fig:difftypes). The code "combinatorial" again denoted combinatorial gesture sequences. For example, children would first make a hammering motion (for "hammer") and then, in a seperate gesture, horizontally stretch out their arms to the side (for "big"). We coded "enact" when children enacted the size of the referent, usually by making bigger movements when gesturing. For example, for the small hammer, the child would make an up and down hammering movement in front of their chest while for the large hammer, they would first lift their arm over their head and then move it all the way down to their hip. A small number of children used the the speed with which they made the gesture to denote size. For example, for the small hammer, they would hammer in a normal pace while hammering more quickly in case of the large hammer. We used the code "tempo" for this. Overall, combinatorial gestures were the most frequent. Children were fairly consistent in their gesture use and the majority only used a dingle type (see Figure \@ref(fig:varwithin). The right graph in Figure \@ref(fig:plot8) (C) shows again the proportion of "combinatorial" gestures. Differentiation again enhanced gesture comprehension ($\beta$ = `r round(summary(mnum.1)$coefficients[3,1], 2)`, se = `r round(summary(mnum.1)$coefficients[3,2], 2)` *p* = `r sub('^(-)?0[.]', '\\1.', round(pmsize.1[3,4], 3))`). 
...
trans2_comb <- ggplot() +
  geom_jitter(data = t2p5, aes(x = 1, y = combinatorial, col = Age), position = position_jitterdodge(jitter.width = .2, jitter.height = 0.05,dodge.width = 0.5), alpha = .5)+
...
trans3_comb <- ggplot() +
  geom_jitter(data = t3p5, aes(x = 1, y = combinatorial, col = Age), position = position_jitterdodge(jitter.width = .2, jitter.height = 0.05,dodge.width = 0.5), alpha = .5)+
...
In the following, we present additional analysis on the combined data from part2. We want to address the following questions:

*What affects children's rate of differentiation?
*When are combinatorial sequences rather than holistic gestures used?
*How do combinatorial sequences affect comprehension?
*Are children efficient in their gesture use?
*Is the structure in children's gesture sequences influenced by the structure in their native language?

For each question, we rely on a different part of the data. Given that we don't have data for transitive 2 and 3 for 6-year-olds, in each section, we will first use data from movement, number and size to look at age differences. We will then repeat the same analysis with data from 8-year-olds for all conditions. Further details on data subsetting can be found in the beginning of each section. 

### Differentiation

Here we look at the factors influencing the likelihood that children differentiate between depictions of the same referent with varying properties. As noted above, differentiation in movement, number and size was a realtive code. Nevertheless, if children used different gestures for the same referent but with different properties, both gestures were coded as differentiated. the following analysis therfore include all trials. 

#### Differentiation by age

```{r diff by age, cache = T}
...
mdiff_age_data <- diff_age_data%>%
  mutate(trial = scale(as.numeric(trial), center = TRUE, scale=TRUE),
         diff = ifelse(diff == "yes",1,0))

# excluded due to convergence issues: block and (1 | stimulus) 
mdiff_age <- glmer(diff ~ Age + condition + trial + block + (trial | id) + (1 | stimulus), family = binomial, data = mdiff_age_data, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
To look at age differences, we exlcude the transitive 2 and 3 conditions. Otherwise, all data is included. We ran the following model: `diff ~ age + condition + trial +  (1 | id) + (1 | stimulus)`. The inclusion of trial allows us to see if children were more likely to differentiate in later trials. The model would not converge if we include block as a fixed effect. However, given that block had no affect on comprehension in any of the analysis we did seperately for each condition, we think it makes sense to assume that children also transferred their way of differentiating to new pictures. We also had to exclude stimulus as a random effect due to convergence issues.

Figure \@ref(fig:diff) shows that children differentiated in the majority of trials across conditions. Eight-year-olds were more likely to differentiate compared to 6-year-olds ($\beta$ = `r round(summary(mdiff_age)$coefficients[2,1], 2)`, se = `r round(summary(mdiff_age)$coefficients[2,2], 2)` *p* = `r sub('^(-)?0[.]', '\\1.', round(pdiff_age[2,4], 3))`). Overall, there was a trend for different rates of differentiation across conditions (LRT for condition: *p* = `r sub('^(-)?0[.]', '\\1.', round(pdiff_age[3,4], 3))`), with highest rates in movement (see also Figure \@ref(fig:diff)).




Children were more likely to differentiate in later trials ($\beta$ = `r round(summary(mnum.1)$coefficients[3,1], 2)`, se = `r round(summary(mnum.1)$coefficients[3,2], 2)` *p* = `r sub('^(-)?0[.]', '\\1.', round(pmsize.1[3,4], 3))`)

Trial makes sense because we use all the data from a all children

Results: Children differentiate more when they are older and in later trials (but no difference between conditions). 

Older children differentiate more. 

No clear effet of condition
...
mdiff_8_data <- diff_8_data%>%
  mutate(trial = scale(as.numeric(trial), center = TRUE, scale=TRUE),
         diff = ifelse(diff == "yes",1,0))
  
# condition as additional random effect has estimate and variance of 0 and is therefore removed
mdiff_8 <- glmer(diff ~ condition + block + trial +  (1 | id) + (1 | stimulus), family = binomial, data = mdiff_8_data, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

summary(mdiff_8)

#plot_model(mdiff_age, type = "re", sort.est = "(Intercept)")

pdiff_8 <- drop1(mdiff_8, test = "Chisq")

```

8yo are better in later trials and also during variations. No effect of condition


```{r diff, fig.cap="Proportion of trials in which children differentiated between depictions of the same referent with different properties by condition and age."}
...
### Combinatorial gesture sequences

Here we subset the data to the trials in which children differentiated. For movement, number and size, the differentiation type was a realtive code and we only coded it for the newly inroduced pictures (i.e. big, moving or multiple referents). Therefore, we only select the trials with these pictures. 

WE take out trial and block as fixed effects because data is heavily subsetted

We did not predcit an effect of condition in the analysis for both age groups with only the modifiers to indicate movement, number or size. However we do predict a condition effect for 8yo. We expected more combiantorial sequences in transitive 2 and 3 comapred to the other conditions

#### Combinatorial gesture use by age
...
# the column hol_or_atom denotes whetehr a given gesture was a holistic (0) or atomistic (1) solution. Atomistic means combinatorial 

mcomb_age <- glmer(hol_or_atom ~ Age + condition + (1|id) + (1 | stimulus), family = binomial, data = mcomb_age_data, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

summary(mcomb_age)

#library(sjPlot)

#plot_model(mcomb_age, type = "re", sort.est = "(Intercept)", ci.lvl = 0)

pcomb_age <- drop1(mcomb_age, test = "Chisq")

# fitting lm with data aggregated for each individual

mcomb_age_data_a <- comb_age_data%>%
  group_by(block)%>%
  mutate(trial = scale(as.numeric(trial), center = TRUE, scale=TRUE))%>%
  group_by(Age, condition,id)%>%
  summarise(mean_atom = mean(hol_or_atom))

mcomb_age_a <- lm(mean_atom ~ Age + condition, data = mcomb_age_data_a)

summary(mcomb_age_a)

drop1(mcomb_age_a,test = "Chisq")


x <- residuals(mcomb_age_a)

ggplot() +
  geom_density(aes(x = x))
```

GLMM suggests no effect of age on combinatorial gesture use. However, the variance on the random effect for id is gigantic. The reason might be that almost all cases are extreme, in the sense that most individuals either always use combinatorial solutions or never. This results in extreme estimates for individual intercepts. This variation over shadows differences between individuals due to age. 

We therefore aggregated data for each individual and ran a simple linear model, predicting proportion of combinatorial gestures by age and condition. This suggests an effect of age. 

Taken together, 8yo might use more combinatorial gestures but this interpretatio

#### Combinatorial gesture use in 8yo

```{r comb 8yo, cache = T}

comb_8yo_data <- gram_data %>%
  mutate(condition = relevel(condition, ref = "transitive_3"))%>%
   filter(Age != "6yo",
          diff == "yes",
          predicate != "small",
          predicate != "static",
...
# model does not converge when random effects are included, we therefore retreat to an lm 

#mcomb_8yo <- glmer(hol_or_atom ~ condition + (1|id) + 1(|condition) , family = binomial, data = comb_8yo_data, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

# fitting lm with data aggregated for each individual

mcomb_8yo_data_a <- comb_8yo_data%>%
  group_by(condition,id)%>%
  summarise(mean_atom = mean(hol_or_atom))

mcomb_8yo_a <- lm(mean_atom ~ condition, data = mcomb_8yo_data_a)

summary(mcomb_8yo_a)

drop1(mcomb_age_a,test = "Chisq")

# residuals look roughly normally distributed
...
GLMMs with id random effect would not converge. Again we aggregated data for each individual and ran a simple lm. Main effect of condition. Transitive 2 and size are not different from trasnitive 3 but movement and number are.

This partly confirms our predictions. Why did size yield so many combinatorial gestures? At this point we don't know and we did not predict this. we though that enacting size would have been fairly straight forward. For example, small hammer = move arm only a little bit, big hammer, move arm quite a lot. 
...
### Effect of combinatorial solutions on comprehension

Combinatorial gestures are more efforful in that children need to depict several elements after one another. Here we look at the influence of combinatorial gestures on comprehension. 

We select only trials with gestures that got a typediff. Because by default the reference pictures did not get a typediff for number size and gesture, they are removed as well. 

We classify all gestures into whether they are combinatorial or holistic.

#### Effect of combinatorial solutions on comprehension by age
...
m.hol_comb <- glmer(compr ~ typediff_1 + condition + Age + ( 1 | id) + (1 | stimulus), family = binomial, data = d.hol_comb, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
Both types are understood very well but combinatorial gestures are understood better. No effect of age on gesture comprehension of differentiated gestures. Effect of condition (see figure)

#### Effect of combinatorial solutions on comprehension in 8yo

Here we exclude transitive 3 because it was not possible to solve this in a holistic way. 
...
m.hol_comb_8 <- glmer(compr ~ typediff_1 + condition + ( 1 | id) + (1 | stimulus), family = binomial, data = d.hol_comb_8, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

summary(m.hol_comb_8)

drop1(m.hol_comb_8, test = "Chi")

```

same effects as above. Combiantorial gestures are understood better. 
...
### Efficiency of gesture use

How efficiently do children use their gestures?

For movement, number and size, whenever they are using a combinatorial gesture sequence, in order to be 100% correct, one needs to modify only one type of referent. This corresponds to a marking rate of 50%.

In the case of trasnsitive actions, all pictures have to identified individually. However, in the case of transitive 2, the patient might be omitted. Therefore, if children were efficient, they should use fewer elements in transitive 2 comapred to transitive 3. 

#### Marked gestures for movement, number and size

```{r}
d.eff_comb <- gram_data  %>%
  filter(condition != "transitive_2",
         condition != "transitive_3")%>%
  mutate(typediff_1 = plyr::mapvalues(typediff_1,c("numges","sizeges","moveges"),rep("combinatorial",3)),
         typediff_1 = factor(ifelse(typediff_1 == "combinatorial", "combinatorial", "holistic")))%>%
  group_by(Age,condition,agent,id)%>%
  mutate(sel = "combinatorial" %in% typediff_1)%>%
  filter(sel == TRUE)%>%
  group_by(block)%>%
  mutate(trial = scale(as.numeric(trial), center = TRUE, scale=TRUE),
         use_typediff = 1- as.numeric(is.na(typediff_1)))%>%
  filter( is.na(typediff_1) | typediff_1 != "holistic")

# test if children mark gestures in more than 50% of trials? Test if intercept is 0
m.eff_comb.1 <- glmer(use_typediff ~ 1 + (1 | id) + (1 | stimulus), family = binomial, data = d.eff_comb, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

summary(m.eff_comb.1)

# test if efficiency is further influence by age and condition 
m.eff_comb.2 <- glmer(use_typediff ~ Age + condition + (1 | id) + (1 | stimulus), family = binomial, data = d.eff_comb, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

summary(m.eff_comb.2)

drop1(m.eff_comb.2, test = "Chisq")
#plot_model(m.eff_comb, type = "re", sort.est = "(Intercept)")


```

Children mark gesture in more than 50% of the time, suggesting they are not terribly efficient 

This is not influenced by other variabels

#### Number of elements used to depict transitive actions 

```{r}

elements_by_condition_md <- gram_data %>%
  filter(condition == "transitive_2" | condition == "transitive_3",
         hol_or_atom == "1")%>%
  group_by(block)%>%
  mutate(trial = scale(as.numeric(trial), center = TRUE, scale=TRUE))


m.el_by_con <- glmer(nr_elements ~ condition + (1 | id) + (1 | stimulus), family = poisson, data = elements_by_condition_md, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
# number of elements (for elaborated versions) and when differentiated

elements_by_condition <- gram_data %>%
  filter(condition == "transitive_2" | condition == "transitive_3",
         hol_or_atom == "1")%>%
  group_by(Age, condition,id)%>%
  summarise(nr_elements = mean(nr_elements))

elements_by_condition_2 <- elements_by_condition %>%
  group_by(Age,condition)%>%
  multi_boot_standard(col = "nr_elements")

ggplot() +
  geom_jitter(data = elements_by_condition, aes(x = condition, y = nr_elements, col = Age), alpha = .5, width = .1,height = .05)+
  geom_pointrange(data = elements_by_condition_2,aes(x = condition, y = mean,ymin = ci_lower, ymax = ci_upper, col = Age),position = position_dodge(width = .5), size = 0.7) +
  guides(alpha = F)+
...
This type of efficiency might be a concern at later stages 


### Word order

Here we ask whether the the internal structure children impose on their gesture sequences is influenced by their native language. It might be that children first translate the picture into a representation and then translate the elements of this representation into gestures. If this were the case, this would be evidence against the idea that children structure their gestures for communicative reasons as they would get the structure "for free" by relying on the structure of their native language. The corresponding prediction would be that the word order in childrens' combinatorial gestures should mostly, if not always, follow the order used in their native language (German). We therefore looked at the order of elements for all gesture squences. For movement, number and size, the corresponding German word order would be P-S (movement: "hüpfender Elefant" [jumping elephant], number: "viele Hämmer" [many hammers], size: "kleine Gabel" [small fork]). In the case of transitive 2 and 3, it would be SPO ("Hase jagt Katze" [bunny chases cat]). 

For this analysis we excluded gestures coded as "combinatorial_part". While we do think that these gestures are combinatorial in that the meaning expressed by the sequence depends on its internal structure, the fact that the predicate was depicted together with the subject and/or the object makes an ordering of these components difficult. Consequently, we could only look at the order of S and O in these gestures. Here we would expect a strong bias in favor of an SO order independent of German word order (see e.g. @greenberg2010language, or @gildea2010grammars).
...
We selected all trials for chich we coded a combinatorial gesture sequence. We excluded trials for which we coded "combinatorial_part". For each trial in the remaining data we coded whether the word order was the same as in German (either P-S or S-P-O). 

```{r}
# unusally large id variation
#yy <- glmer(lang_con_order ~ 1 + (1 | id) + (1 | stimulus) , family = binomial, data = worder_data, control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

#summary(yy)

#source("boot_glmm.r")

#yy.ci <- boot.glmm.pred(model.res = yy, excl.warnings = F, nboots = 1000,
...
t.test(worder_data%>%
  group_by(id)%>%
  summarise(mean = mean(lang_con_order))%>%pull(mean), mu = 0.5)
...
mean(worder_data$lang_con_order)

```

Again, very large variance on random intercept for id, suggesting that children were relatively consistent in the kind of word order they used (see also distribution in \@ref(fig:wordorder)). Like previously, we aggregated the data and run a simple t-test against chance. This shows a small but significant bias for german word order. However, the confidence interval around the mean also does not include 1, suggesting that the usage of non-german word order is not attributable to error but systematic.

```{r wordorder, fig.cap="Proportion of German word order in childrens combinatorial gestures. This excludes 'combinatorial_part' for reasons outlined in the main text." }
